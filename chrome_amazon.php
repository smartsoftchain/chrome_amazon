...
//amazon取得
$sql = "select `key`,`secret` from `api` where `type`=2 order by `error` limit 1";
$ret = mysql_query($sql,$cn);
if($ret){
...
//echo $awsk;
	...
	$error = 1;
	$srror_str .= "Amazon-key:error<br />";
}
//楽天取得
$sql = "select `key`,`aid` from `api` where `type`=0 order by `error` limit 1";
$ret = mysql_query($sql,$cn);
if($ret){
	while($rows = mysql_fetch_array($ret)){
		$rakukey = $rows["key"];
		define("RAKUTEN_ID",$rows["aid"]);
	}
}else{
	$error = 1;
	$srror_str .= "Rakuten-key:error<br />";
}
//Yahoo取得
$sql = "select `key`,`sid`,`pid` from `api` where `type`=1 order by `error` limit 1";
$ret = mysql_query($sql,$cn);
if($ret){
	while($rows = mysql_fetch_array($ret)){
		define("YAHOO_SHOP_SID",$rows["sid"]);
		define("YAHOO_SHOP_PID",$rows["pid"]);
		$yahookey = $rows["key"];
	}
}else{
	$error = 1;
	$srror_str .= "Yahoo-key:error<br />";
}
	...
	if($mode == "amazon"){
		...
		if($amazon["ItemAttributes"]["EAN"]){
			...
			if($amazon2){
				...
				if($c%2!=0){$c=$c+1;}
				$c2 = $c/2;
				foreach($amazon2 as $key => $val){
					$str .= "<tr><td>".$cnt."</td><td><a target=\"_blnak\" href=\"".$val["urls"]."\">".$val["subStoreName"]."</a></td><td align=\"right\">".number_format($val["price"])."</td>";
					$str .= "<td>".($cnt+$c2)."</td><td><a target=\"_blank\" href=\"".$amazon2[($cnt+$c2)]["urls"]."\">".$amazon2[($cnt+$c2)]["subStoreName"]."</a></td><td align=\"right\">".number_format($amazon2[($cnt+$c2)]["price"])."</td></tr>";
	...
	if($error == 0){
		...
		echo "<a href=\"chrome_amazon.php?mode=".$mode."&asin=".$asin."&countries=".$countries."&countries_used=".$countries_used."\"><input type=\"button\" value=\"再読込み\"></a>";
...
function SearchAmazon($asin){
	global $awsk,$awss,$cn,$amazon_ast;
	...
	require_once 'Services/Amazon.php';
		...
		if($type=="asin"){
			...
			if(@get_class($response)!="PEAR_Error"){
				...
				$sql = "update `api` set `error`=`error`+1 where `key`='".$awsk."'";
				@mysql_query($sql,$cn);
				return array();
			}
